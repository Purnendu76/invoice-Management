...
export default function InvoiceForm({
  ...
  const states = ["West Bengal", "Delhi", "Bihar", "MP", "Kerala", "Sikkim", "Jharkhand", "Andaman"];
  const billCategories = ["Service", "Supply", "ROW", "AMC", "Restoration Service", "Restoration Supply", "Restoration Row", "Spares", "Training"];
  const milestones = ["60%", "90%", "100%"];
  const gstOptions = ["5%", "12%", "18%"];
  const statuses = ["Paid", "Cancelled", "Under process", "Credit Note Issued"];

  // State variables
  const [project, setProject] = useState<string | null>(null); // auto-filled
...
useEffect(() => {
  const fetchUserProject = async () => {
    ...
    try {
      ...
      // Map the backend field to frontend
  ...
  usePrefillInvoiceForm({
    initialValues,
    setters: {
      setInvoiceNumber, setInvoiceDate, setSubmissionDate, setBasicAmount,
      setGstPercentage, setTotalAmount, setPassedAmount, setRetention,
      setGstWithheld, setTds, setGstTds, setBocw, setLowDepth, setLd,
      setSlaPenalty, setPenalty, setOtherDeduction, setStatus, setAmountPaid,
      setPaymentDate, setRemarks, setProject, setMode, setState, setBillCategory,
      setMilestone
    },
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!project || !mode || !state || !billCategory || !invoiceNumber || !invoiceDate || !submissionDate || !basicAmount || !gstPercentage || !status) {
    ...
    const payload = {
      project, modeOfProject: mode, state, mybillCategory: billCategory, milestone,
      invoiceNumber,
    ...
    try {
      ...
      let res;
      if (initialValues?.id) {
        res = await axios.put(`/api/v1/user-invoices/${initialValues.id}`, payload, { headers: { Authorization: `Bearer ${token}` } });
        notifySuccess("Invoice updated successfully ✅");
      } else {
        res = await axios.post("/api/v1/user-invoices", payload, { headers: { Authorization: `Bearer ${token}` } });
        notifySuccess("Invoice submitted successfully ✅");
      }
      if (onSubmit) onSubmit({ client: billCategory||"Unknown", amount: netPayable });
      if (onClose) onClose();
      ...
      <form onSubmit={handleSubmit}>
        <Stack>
          ...
          <Group justify="flex-end" mt="md">
            <Button size="sm" type="submit" disabled={loading}>{loading?"Submitting...":"Submit"}</Button>
          </Group>
        </Stack>
      </form>
    </Box>
